...
{
	return float4(0.75f ,0.75f, 0.25f, 0);
}
)";
...
public:
	Renderer(GW::SYSTEM::GWindow _win, GW::GRAPHICS::GVulkanSurface _vlk)
	{
		...
		GvkHelper::create_buffer(physicalDevice, device, sizeof(vertices), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &vertexBuffer, &vertexMemory);
...
private:
	...
	void ShaderToSpirV()
	{
		// Intialize runtime shader compiler HLSL -> SPIRV
		//Create Vertex Shader
		shaderc_compilation_result_t result = shaderc_compile_into_spv(compiler, vertexShaderSource, strlen(vertexShaderSource), shaderc_vertex_shader, "main.vert", "main", options);
		...
		//Create Pixel Shader
		result = shaderc_compile_into_spv(compiler, pixelShaderSource, strlen(pixelShaderSource), shaderc_fragment_shader, "main.frag", "main", options);
	...
	void FileCheck(const char* filePath, std::string shader)
	{
		std::string shaderString = ShaderAsString(filePath);
		if (shader != shaderString)
		{
			hotload = true;
		}
	}
	...
	// Simple Vertex Shader
	std::string verShader = ShaderAsString("../Shaders/VertexShader.hlsl");
	const char* vertexShaderSource = verShader.c_str();
	// Simple Pixel Shader
	std::string pixShader = ShaderAsString("../Shaders/PixelShader.hlsl");
	const char* pixelShaderSource = pixShader.c_str();

	bool hotload = false;

	std::thread pixelChecker;
	std::thread vertexChecker;
};